...
<body> 
	...
	<form action="mini index.php" method="get">
	<table width="100%">
	<tr>
		<td>
		<input type="button" name="login" value="Login" onclick="location.href='mini login.html'"></td><td>
	<input type="text" name='search' placeholder='SEARCH BY CATEGORY...'> </td><td>
	<input type="submit" name="search" value="Search"></td><td>
	<a href="mini register.html"><input type="button" name="register" value="Register"></a></td></tr>
	</table></form>
...
//$result=mysql_query("select * from item where company='Rado'",$con); 
while($row=mysql_fetch_array($result))
{	
	$id=$row['id'];
	echo "<table width='100%' class='item'> <br>";
	echo "<tr ><td width='25%'><a href='mini item.php?id=$id'><center><img src='";
	echo $row['photo']."'></img>";
	echo "</center></a></td><td style=".'"font-size="50px"'."><a href='mini item.php?id=$id'><center><u><b>";
